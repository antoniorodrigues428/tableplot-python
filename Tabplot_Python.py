...
def simulate_tabplot_binning(df, nbins=100, sort_col=None, decreasing=True, max_levels=50):
    ...
    # CORRIGIDO: ordem natural dos bins (menor no topo, maior na base)
...
file_path = r"C:\Users\Dell\Downloads\Teste999.xlsx"  # Altere aqui conforme seu arquivo
...
try:
    ...
    print("\n--- Colunas Disponíveis no Dataset (Índice | Nome) ---")
    for i, col in enumerate(df_original.columns):
        print(f"[{i+1}] {col}")
    print("---------------------------------------------------\n")

    # DEFINIÇÃO FIXA DAS COLUNAS A SEREM PLOTADAS (1 a 7)
    # Garante que as colunas selecionadas são as primeiras do DataFrame original
    col_indices_to_plot = [0, 1, 2, 3, 4, 5, 6] 
    
    # Validação dos índices para garantir que não excedem o DataFrame
    if any(idx < 0 or idx >= len(df_original.columns) for idx in col_indices_to_plot):
        raise ValueError("Um ou mais índices de coluna definidos para plotagem estão fora do intervalo válido do DataFrame.")
    
    selected_columns_names = [df_original.columns[idx] for idx in col_indices_to_plot]
    df_selected_for_plot = df_original[selected_columns_names].copy()

    # DEFINIÇÃO FIXA DA COLUNA DE ORDENAÇÃO (coluna 1, que é o índice 0)
    sort_col_idx = 0 
    
    # Validação do índice da coluna de ordenação
    if sort_col_idx < 0 or sort_col_idx >= len(df_original.columns):
        raise ValueError("O índice da coluna de ordenação definido está fora do intervalo válido do DataFrame.")
    
    initial_sort_col_name = df_original.columns[sort_col_idx]
    
    # Não há mais renomeação de colunas. A referência sempre será o nome original.

    print("\nColunas selecionadas para plotagem:", df_selected_for_plot.columns.tolist())
    print("Coluna usada para ordenação:", initial_sort_col_name)

    # Garantir que as colunas booleanas/categóricas são do tipo correto, e com NaN
    # Usando os nomes originais das colunas
    if 'Is New Member' in df_selected_for_plot.columns: 
        df_selected_for_plot['Is New Member'] = df_selected_for_plot['Is New Member'].astype(str).replace({
    ...
    if 'Product Type' in df_selected_for_plot.columns: 
        df_selected_for_plot['Product Type'] = df_selected_for_plot['Product Type'].astype(str).replace({'nan': np.nan}).astype('category')

    # Chamar a função de simulação de binning
    binned_data_processed, _, _ = simulate_tabplot_binning(
        df_selected_for_plot, nbins=100, sort_col=initial_sort_col_name, decreasing=False 
    )

    # Criar o título dinâmico com base no nome do arquivo
    base_filename = os.path.basename(file_path)
    file_name_without_extension = os.path.splitext(base_filename)[0]
    dynamic_title = f"Tabplot from: {file_name_without_extension}" 

    # Chamar a função de plotagem com o título dinâmico
    plot_tabplot_interactive(binned_data_processed, title=dynamic_title)
